...
import { useDeleteProposal } from "@/api/democracy/proposals";
import useDisclosure from "@/hooks/useDisclosure";
import ALert from "@/components/custom/Alert";
import { useAuthContext } from "@/providers/AuthProvider";
import { frontendURL } from "@/api/baseUrl";
// import { useAppContext } from "@/contexts/AppContext";
import SDGCard from "../common/SDGCard";
...
const ProposalInfo: React.FC<ProposalInfoProps> = ({
  ...
  return (
    <div className="flex flex-col justify-start gap-10 xl:flex-row">
      <div className=" w-full xl:min-w-[700px] flex flex-col gap-6">
        {/* MAIN INFO */}
        <div className="flex flex-col w-full">
          <h1 className="py-3 text-2xl capitalize lg:text-3xl text-dark">
            {proposal.title}
          </h1>
          ...
          {/* SUMMARY */}
          <div className="py-4 border-t-2 border-subtle_text">
            <h2 className="text-base">
              Proposal code: <span>{proposal.proposal_code}</span>
            </h2>
            <p className="py-2 text-sm md:text-base">{proposal.summary}</p>
          </div>

          {/* BODY */}
          <div className="w-full py-4 overflow-hidden whitespace-pre-wrap border-t-2 border-subtle_text">
            <div
          ...
          <div className="py-4 my-8 px-2 max-w-[700px] flex-wrap bg-light border border-primary rounded-lg flex justify-between items-center">
            ...
            <Button className="relative p-0 bg-transparent w-fit h-fit hover:bg-transparent">
              <Copy
                size="25"
                color="#292925"
                variant="Bold"
              ...
              />
              {copied && (
                <p className="absolute text-sm -left-12 text-primary">Copied</p>
              )}
            </Button>
          </div>
        </div>
              ...
              <div>
                ...
                >
                  <Link
                    ...
                    className="flex justify-center w-full h-full gap-3 px-8 py-3"
                  >
                    <span>Edit</span>
                    <CardEdit />
                  </Link>
        ...
        {isLoggedIn && (
          <>
            {/* <div>
              ...
              <Button className="h-12 mt-3 bg-transparent border border-primary text-primary hover:text-light">
